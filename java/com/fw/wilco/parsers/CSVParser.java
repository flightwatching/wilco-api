...
public class CSVParser implements IParser {
	...
	enum MANDATORY_FIELDS {FWOT, DATE, TIME};
	...
	public List<InputMessageV3IO> parse(File file) throws ParseException {
		...
		try {
			...
			if (!it.hasNext()) {
				throw new ParseException("header", "the format must at least have a header (see WILCO CSV format)");
			}
			CSVRecord header = it.next();
			List<String> params = new ArrayList<String>();
			//the first is the A/C, the second the date and then the records
			...
			while (it.hasNext()) {
				...
				String fwot = csvRecord.get(0).trim();
				if (m.reg!=null && !m.reg.equals(fwot)) {
